...
Use the `agent-browser` CLI skill instead of installing `chrome-devtools-mcp`. The MCP registry in this project does not include a Chrome DevTools server; the canonical browser-automation entry point is the `agent-browser` skill (CLI, not MCP).
...
`agent-browser` exposes these capabilities via its CLI subcommands:

| Capability | What It Does | When to Use |
|------|-------------|-------------|
| **Snapshot** | Captures the current page state | Visual verification, before/after comparisons |
