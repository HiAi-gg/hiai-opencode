...
- `agent-browser` — browser automation via CDP on Lightpanda (preferred when installed) or, on upstream hosts, Chrome (uses `/agent-browser` skill)
...
- **This workstation (DEV-01):** Lightpanda only. Operator docs must not run `agent-browser install` (that provisions Chrome). See [AGENTS.md](AGENTS.md).
...
- **Upstream-host fallback:** **Chrome**, `agent-browser`'s own default engine, provisioned by `agent-browser install` on hosts that opt into Chrome. Not a HiAi DEV-01 requirement.
...
**Unsupported on Lightpanda** (do not claim visual/runtime coverage from these): headed mode (`AGENT_BROWSER_HEADED=1`), extensions, profiles, persistent auth state, local `file://` access, Chrome DevTools MCP / GPU flags. Screenshot fidelity is engine-dependent. A `test:e2e` placeholder is not browser coverage.
