...
export function getTaskDir(config: Partial<HiaiOpenCodeConfig> = {}): string {
  const tasksConfig = config.bob?.tasks
...
export function resolveTaskListId(config: Partial<HiaiOpenCodeConfig> = {}): string {
  ...
  const configId = config.bob?.tasks?.task_list_id?.trim()
...
export function listTaskFiles(config: Partial<HiaiOpenCodeConfig> = {}): string[] {
