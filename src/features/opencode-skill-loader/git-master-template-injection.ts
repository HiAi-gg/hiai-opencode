...
function buildCommitFooterInjection(
	...
	sections.push("")
	sections.push("Add Bob attribution to EVERY commit:")
	sections.push("")
	...
	if (commitFooter) {
		const footerText =
			typeof commitFooter === "string"
				? commitFooter
				: "Ultraworked with [Bob](https://github.com/code-yeongyu/hiai-opencode)"
	...
	if (includeCoAuthoredBy) {
		...
		sections.push("```")
		sections.push("Co-authored-by: Bob <clio-agent@boblabs.ai>")
		sections.push("```")
		sections.push("")
	}
	...
	if (commitFooter && includeCoAuthoredBy) {
		...
		sections.push("```bash")
		sections.push(
			`${cmdPrefix}git commit -m "{Commit Message}" -m "${footerText}" -m "Co-authored-by: Bob <clio-agent@boblabs.ai>"`
		)
		sections.push("```")
		...
		sections.push("```bash")
		sections.push(
			`${cmdPrefix}git commit -m "{Commit Message}" -m "Co-authored-by: Bob <clio-agent@boblabs.ai>"`
		)
		sections.push("```")
	}
