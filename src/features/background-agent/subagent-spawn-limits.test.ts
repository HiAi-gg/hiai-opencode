...
describe("Task 2.2: Delegation limits", () => {
  ...
  describe("getMaxRootSessionSpawnBudget", () => {
    ...
    test("returns default when config has no maxDescendants", () => {
      const config: BackgroundTaskConfig = {}
      expect(getMaxRootSessionSpawnBudget(config)).toBe(DEFAULT_MAX_ROOT_SESSION_SPAWN_BUDGET)
    })
  })
  ...
  describe("resolveSubagentSpawnContext", () => {
    test("resolves root session for top-level session", async () => {
      ...
      const context = await resolveSubagentSpawnContext(client as any, "session-1")
    ...
    test("resolves root session through parent chain", async () => {
      ...
      const context = await resolveSubagentSpawnContext(client as any, "session-3")
    ...
    test("detects session parent cycle", async () => {
      ...
      await expect(resolveSubagentSpawnContext(client as any, "session-1")).rejects.toThrow("Detected a session parent cycle")
    })
    ...
    test("handles session get error", async () => {
      ...
      await expect(resolveSubagentSpawnContext(client as any, "session-1")).rejects.toThrow("Subagent spawn blocked")
    })
    ...
    test("handles missing session data", async () => {
      ...
      await expect(resolveSubagentSpawnContext(client as any, "session-1")).rejects.toThrow("No session data returned")
    })
  })
  ...
  describe("createSubagentDepthLimitError", () => {
    test("creates error with correct message", () => {
      ...
      expect(error.message).toContain("child depth 4 exceeds background_task.maxDepth=3")
  ...
  describe("createSubagentDescendantLimitError", () => {
    test("creates error with correct message", () => {
      ...
      expect(error.message).toContain("root session root-1 already has 50 descendants")
      expect(error.message).toContain("background_task.maxDescendants=50")
    })
  })
})

function createMockClient(sessions: Record<string, { parentID: string | null }>) {
  return {
    session: {
