...
import { log, getAgentToolRestrictions, createInternalAgentTextPart, promptWithModelSuggestionRetry } from "../../shared"
...
export class TaskExecutor {
  ...
  private get directory() { return this.deps.directory }
  private get state() { return this.deps.state }
  private get concurrencyManager() { return this.deps.concurrencyManager }
  private get taskHistory() { return this.deps.taskHistory }
  private get tmuxEnabled() { return this.deps.tmuxEnabled }
  private get onSubagentSessionCreated() { return this.deps.onSubagentSessionCreated }
  ...
  private async abortSessionWithLogging(sessionID: string, reason: string): Promise<void> {
    try {
      ...
      log(`[background-agent] abortSessionWithLogging failed (${reason}):`, error)
    }
  }
  ...
  private rollbackPreStartDescendantReservation(task: BackgroundTask): void {
    if (task.rootSessionID) {
      if (this.state.preStartDescendantReservations.has(task.id)) {
        const count = this.state.rootDescendantCounts.get(task.rootSessionID) ?? 0
        if (count > 0) {
