...
export function enqueueNotificationForParent(
  ...
  const previous =
    adapter.notificationQueueByParent.get(parentSessionID) ?? Promise.resolve();
...
export async function notifyParentSession(
  ...
  if (pendingSet) {
    ...
    remainingCount = Array.from(adapter.tasks.values()).filter(
      (t) =>
  ...
  const completedTasks = allComplete
    ? (adapter.completedTaskSummaries.get(task.parentSessionID) ?? [
        {
          id: task.id,
          description: task.description,
          status: task.status,
  ...
  if (adapter.enableParentSessionNotifications) {
    try {
      const messagesResp = await adapter.client.session.messages({
    ...
    try {
      if (hasPrompt) {
        ...
        try {
          const prompt = adapter.client.session.prompt.bind(
        ...
        log(
          "[background-agent] prompt() not available on session, cannot send notification:",
          {
            taskId: task.id,
            parentSessionID: task.parentSessionID,
          },
      ...
      if (
        ...
        isRecoverablePromptInjectionError(error)
      ) {
        log(
          "[background-agent] Parent session aborted while sending notification; queuing for retry:",
          {
            taskId: task.id,
            parentSessionID: task.parentSessionID,
            recoverable: true,
