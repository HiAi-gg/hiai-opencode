...
function expandPluginRootPlaceholders(
  value: unknown,
  pluginRoot: string,
): unknown {
...
function requireModelSlots(
  ...
  if (missing.length > 0) {
    throw new Error(
      `[hiai-opencode] Missing required model slot(s) in hiai-opencode.json: ${missing.join(", ")}`,
    );
  }
  return resolved;
}
...
function deriveCategories(
  _models: Record<ModelSlot, string>,
): Record<string, import("./types.js").CategoryConfig> {
...
function deriveMcp(config: HiaiOpencodeConfig): HiaiOpencodeConfig["mcp"] {
  ...
  return Object.fromEntries(
    Object.entries(defaults).map(([name, entry]) => {
      ...
      if (
        name === "mempalace" &&
        typeof merged.pythonPath === "string" &&
...
function loadBundledDefaultConfig(): HiaiOpencodeConfig {
  ...
  return expandPluginRootPlaceholders(
    materialized,
    pluginRoot,
  ) as HiaiOpencodeConfig;
}

export function applyModelSlots(
  config: HiaiOpencodeConfig,
): HiaiOpencodeConfig {
