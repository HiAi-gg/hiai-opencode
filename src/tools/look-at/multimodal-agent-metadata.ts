...
function _isConfiguredVisionModel(
  ...
  try {
    ...
    if (registeredMetadata.agentModel) {
      ...
      if (registeredModelIsVisionCapable) {
        log("[look_at] Using registered ui model (vision-capable)", {
      ...
      log(
        "[look_at] Registered ui model not in vision-capable cache, using it anyway",
        {
          model: getFullModelKey(registeredMetadata.agentModel),
        },
      );
    ...
    log("[look_at] Failed to resolve ui model info", error);
    return {};
  }
}
