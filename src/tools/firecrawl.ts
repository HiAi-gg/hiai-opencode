 ...
 */
export function formatFirecrawlError(stderr: string, stdout: string, message: string): string {
...
async function runFirecrawl(args: string): Promise<string> {
  try {
    ...
    if (err.stdout) {
      return formatFirecrawlError(err.stderr || '', err.stdout, err.message || '');
    }
    const classified = formatFirecrawlError(err.stderr || '', '', err.message || '');
    return classified;
  }
}

export const firecrawlSearchTool = tool({
  description: 'Search the web using Firecrawl. Returns structured, LLM-optimized results.',
  args: {
