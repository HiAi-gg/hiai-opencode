...
function convertBuiltinMcpConfig(
...
function formatAvailableBuiltinMcps(
  builtinMcp: Record<string, McpServerConfig> | undefined,
): string {
...
export function createSkillMcpTool(
  ...
  return tool({
    ...
    async execute(args: SkillMcpArgs, toolContext: ToolContext) {
      ...
      const convertedBuiltinConfig = builtinConfig
        ? convertBuiltinMcpConfig(builtinConfig)
        : null;
