...
export async function processMessages(
  ...
  if (messagesResult.error) {
    log(`[call_hiai_agent] Messages error:`, messagesResult.error)
    throw new Error(`Failed to get messages: ${messagesResult.error}`)
  }

  const messages = messagesResult.data
  log(`[call_hiai_agent] Got ${messages.length} messages`)
  ...
  if (relevantMessages.length === 0) {
    log(`[call_hiai_agent] No assistant or tool messages found`)
    log(`[call_hiai_agent] All messages:`, JSON.stringify(messages, null, 2))
    throw new Error("No assistant or tool response found")
  }

  log(`[call_hiai_agent] Found ${relevantMessages.length} relevant messages`)
  ...
  log(`[call_hiai_agent] Got response, length: ${responseText.length}`)
