...
export async function resolveOrCreateSessionId(
	...
	if (args.session_id) {
		...
		if (sessionResult.error) {
			log("[call_hiai_agent] Session get error", { error: sessionResult.error })
			return {
				ok: false,
	...
	const parentSession = await ctx.client.session
		...
		.catch((err: unknown) => {
			log("[call_hiai_agent] Failed to get parent session", { error: String(err) })
			return null
		})
	...
	if (createResult.error) {
		log("[call_hiai_agent] Session create error", { error: createResult.error })
