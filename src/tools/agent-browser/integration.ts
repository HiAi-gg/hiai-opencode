...
export function createAgentBrowserIntegrationTool(): AgentBrowserIntegrationToolEntry[] {
  const agent_browser_install: ToolDefinition = tool({
    description:
      "Install agent-browser globally via npm and run browser installation. Use this when agent-browser is not found or needs to be updated. Returns installation output and version check.",
    args: {},
    execute: async () => {
      try {
        const command =
          "npm install -g agent-browser@latest && agent-browser install";
        ...
        if (exitCode !== 0) {
          const errorMsg =
            stderr.trim() || `Installation failed with exit code ${exitCode}`;
          return `Error: ${errorMsg}`;
        }

        const verifyProc = spawn(["agent-browser", "--version"], {
          stdout: "pipe",
          stderr: "pipe",
        });
  ...
  const agent_browser_doctor: ToolDefinition = tool({
    description:
      "Run agent-browser health check in offline quick mode. Checks if the browser daemon is running, Chrome is installed, and configuration is valid. Returns doctor output with pass/fail status for each check.",
    args: {},
    execute: async () => {
      try {
        ...
        if (exitCode !== 0) {
          const errorMsg =
            stderr.trim() || `Doctor check failed with exit code ${exitCode}`;
          return `Error: ${errorMsg}`;
        }
