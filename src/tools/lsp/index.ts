  ...
  async execute(input, ctx) {
    ...
    if (input.directory && !input.filePath) {
      ...
      if (!existsSync(dir) || !statSync(dir).isDirectory()) {
        return { title: 'Invalid directory', output: `Directory not found: ${dir}` };
      }
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          const { client, resolved, serverId } = await getClientForFile(fullPath, ctx.directory);
          ...
          try {
            ...
            results.push(...filtered.map((d) => formatDiagnostic(d, ctx.directory)));
          } catch {
      ...
      return { title: `Directory scan (${results.length} files)`, output: results.join('\n') };
    }
    ...
    const resolvedPath = filePath.startsWith('/') ? filePath : join(ctx.directory, filePath);
    sandboxCheck(resolvedPath, ctx.directory);

    if (!existsSync(resolvedPath)) {
      return { title: 'File not found', output: `File not found: ${resolvedPath}` };
    }

    try {
      const { client, resolved, serverId } = await getClientForFile(resolvedPath, ctx.directory);
      try {
        if (!client) {
          return {
        ...
        const diagnostics = await client.diagnostics(resolved);
        const filtered = maxSev ? filterDiagnosticsBySeverity(diagnostics, maxSev) : diagnostics;

        if (filtered.length === 0) {
          return { title: 'No diagnostics', output: 'No errors or warnings found.' };
        }

        const output = filtered.map((d) => formatDiagnostic(d, ctx.directory)).join('\n');
...
export const lspSymbolsTool = tool({
  ...
  args: {
    ...
    query: tool.schema
      .string()
      .optional()
      .describe('Search query for workspace symbols (used with scope=workspace)'),
    scope: tool.schema
      .string()
      .optional()
      .default('document')
...
export const lspRenameTool = tool({
  ...
  args: {
    ...
    apply: tool.schema
      ...
      .optional()
      .default(false)
      .describe('When true, apply the rename edits to disk (default: preview only)'),
  },
  async execute(input, ctx) {
    const filePath = input.filePath.startsWith('/')
      ? input.filePath
