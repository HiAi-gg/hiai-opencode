...
export function createMemoryTool(memoryService: ReturnType<typeof createMemoryService>) {
  return tool({
    description:
    ...
    args: {
      query: tool.schema.string().describe('Search query — use 1-3 distinctive terms'),
      scope: tool.schema
        .string()
        .optional()
        .describe('Scope filter: "global", "projects", "sessions", or leave empty for all'),
      scope_id: tool.schema.string().optional().describe('Scope ID (project hash or session ID)'),
      type: tool.schema
        .string()
        .optional()
        .describe('Type filter: "memory", "checkpoint", "progress", "notes", "free"'),
    },
    async execute(input) {
      try {
