...
import type { BobAgentConfig } from "../../config/schema";
...
function isStrategistAgent(agentToUse: string): boolean {
  const normalizedAgent =
    stripInvisibleAgentCharacters(agentToUse).toLowerCase();
...
export async function sendSyncPrompt(
  ...
  input: {
    ...
    bobAgentConfig?: BobAgentConfig;
  },
  deps: SendSyncPromptDeps = sendSyncPromptDeps,
): Promise<string | null> {
  const allowTask = isPlanFamily(input.agentToUse);
  const tddEnabled = input.bobAgentConfig?.tdd;
  ...
  try {
    ...
    if (errorMessage.includes("UnknownError")) {
      log(
        `[sendSyncPrompt] UnknownError in OpenCode core for agent "${input.agentToUse}". Category: "${input.args.category}". Model in body: ${input.categoryModel ? "present" : "MISSING"}.`,
      );
    }
    if (
      isStrategistAgent(input.agentToUse) &&
