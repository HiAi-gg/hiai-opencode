...
const CANONICAL_DELEGATE_AGENT_KEY_SET = new Set<string>(
...
export function resolveCanonicalDelegateAgentKey(agentName: string): string {
  ...
  if (resolved && resolved !== configKey) {
    log("[delegate-task] Legacy agent alias resolved", {
      from: configKey,
      to: resolved,
    });
    return resolved;
...
export function isCanonicalDelegateAgentKey(agentName: string): boolean {
  return CANONICAL_DELEGATE_AGENT_KEY_SET.has(
    resolveCanonicalDelegateAgentKey(agentName),
  );
}

export function getCanonicalDelegateAgentDisplayName(
  agentName: string,
): string {
