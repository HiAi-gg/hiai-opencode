...
export async function resolveSubagentExecution(
  ...
  const requestedCanonicalAgentKey =
    resolveCanonicalDelegateAgentKey(requestedAgentName);
...
SubAgent is spawned automatically when you specify a category. Pick the appropriate category for your task domain.`,
  ...
  const canonicalParentAgent = parentAgent
    ? resolveCanonicalDelegateAgentKey(parentAgent)
    : undefined;
  if (
    isPlanFamily(requestedCanonicalAgentKey) &&
    isPlanFamily(canonicalParentAgent)
  ) {
    return {
      agentToUse: "",
      categoryModel: undefined,
...
Create the work plan directly - that's your job as the planning agent.`,
  ...
  if (
    ...
    canonicalParentAgent === requestedCanonicalAgentKey
  ) {
    return {
      agentToUse: "",
      categoryModel: undefined,
  ...
  try {
    ...
    const canonicalAgentConfigKey =
      resolveCanonicalDelegateAgentKey(agentToUse).toLowerCase();
    const agentConfigLookupKeys = Array.from(
      new Set([canonicalAgentConfigKey, matchedAgentConfigKey]),
    );

    const agentOverride = agentOverrides
      ? Object.entries(agentOverrides).find(([key]) =>
          agentConfigLookupKeys.includes(key.toLowerCase()),
        )?.[1]
      : undefined;
    const agentRequirement = agentConfigLookupKeys
      .map((key) => AGENT_MODEL_REQUIREMENTS[key])
      .find(
        (requirement): requirement is NonNullable<typeof requirement> =>
