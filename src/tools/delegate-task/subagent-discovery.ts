...
function matchesRequestedAgent(
  ...
  const listedAgentConfigKey = normalizeComparableName(
    getAgentConfigKey(agent.name),
  );
...
export function findCallableAgentMatch(
  ...
  return selectPreferredAgentMatch(agents, requestedAgentName, (mode) =>
    isTaskCallableAgentMode(mode),
  );
}
...
export function listCallableAgentNames(agents: AgentInfo[]): string {
  ...
  return Array.from(uniqueNames.values()).sort().join(", ");
}
