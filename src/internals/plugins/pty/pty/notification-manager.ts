...
export class NotificationManager {
  ...
  async sendExitNotification(
    ...
    try {
      ...
      const promptFn = (
        this.client.session as unknown as Record<string, unknown>
      )?.prompt;
      if (typeof promptFn !== "function") {
        log(
          "[pty] prompt() not available on session, cannot send exit notification:",
          {
      ...
      try {
        ...
        log("[pty] Sent exit notification:", {
          sessionId: session.id,
          exitCode,
        });
      } finally {
