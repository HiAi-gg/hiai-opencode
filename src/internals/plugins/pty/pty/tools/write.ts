...
function parseEscapeSequences(input: string): string {
  return input.replace(
    /\\(x[0-9A-Fa-f]{2}|u[0-9A-Fa-f]{4}|[nrt\\])/g,
    (match, seq: string) => {
      if (seq.startsWith("x")) {
...
export const ptyWrite: ToolDefinition = tool({
  ...
  args: {
    id: tool.schema
      .string()
      .describe("The PTY session ID (e.g., pty_a1b2c3d4)"),
  ...
  async execute(args) {
    ...
    if (session.status !== "running") {
      throw new Error(
        `Cannot write to PTY '${args.id}' - session status is '${session.status}'.`,
      );
    }
    ...
    const preview =
      args.data.length > 50 ? `${args.data.slice(0, 50)}...` : args.data;
