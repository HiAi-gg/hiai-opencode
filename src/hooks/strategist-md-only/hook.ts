  ...
  return {
    "tool.execute.before": async (
      ...
      if (toolName === "skill") {
        const name =
          typeof output.args.name === "string"
            ? normalizeName(output.args.name)
            : "";
        if (
          BLOCKED_EXECUTION_SKILLS.includes(name) ||
          BLOCKED_EXECUTION_COMMANDS.includes(name)
        ) {
      ...
      if (TASK_TOOLS.includes(toolName)) {
        const category =
          typeof output.args.category === "string"
            ? normalizeName(output.args.category)
            : undefined;
        const subagentType =
        ...
        if (
          subagentType &&
          !ALLOWED_PLANNING_SUBAGENTS.includes(subagentType)
        ) {
        ...
        if (blockedSkill) {
          log(
            `[${HOOK_NAME}] Blocked strategist execution skill in delegation`,
            {
              sessionID: input.sessionID,
              tool: toolName,
              blockedSkill,
      ...
      if (
        normalizedPath.includes(".bob/plans/") ||
        normalizedPath.includes(".bob\\plans\\")
      ) {
