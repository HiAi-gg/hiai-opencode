...
const BROWSER_AUTOMATION_DENY: Array<{ pattern: RegExp; reason: string }> = [
  ...
  { pattern: /firefox\.launch\s*\(/i, reason: 'firefox.launch() is a Playwright API — forbidden' },
  { pattern: /webkit\.launch\s*\(/i, reason: 'webkit.launch() is a Playwright API — forbidden' },
  {
  ...
  { pattern: /puppeteer\.launch\s*\(/i, reason: 'puppeteer.launch() is forbidden' },
  // CLI invocations
  {
  ...
  { pattern: /\bnpm\s+exec\s+playwright\b/i, reason: 'npm exec playwright is forbidden' },
  // Chrome DevTools MCP
  {
  ...
  // Script file patterns
  {
    ...
    reason: 'node ...playwright... path suggests a Playwright script — forbidden',
  },
  {
  ...
  // Paths
  { pattern: /\.cache\/puppeteer\b/i, reason: 'Puppeteer cache path is forbidden' },
  { pattern: /\.cache\/playwright\b/i, reason: 'Playwright cache path is forbidden' },
  {
    pattern: /node_modules\/playwright\b/i,
    reason: 'node_modules/playwright path suggests a Playwright installation — forbidden',
  },
  // Package names with scope (no \b before @ since @ is not a word character)
  {
    pattern: /@playwright\//i,
    reason: '@playwright/* packages are forbidden — use agent-browser via Vision',
  },
  {
    pattern: /@puppeteer\//i,
    reason: '@puppeteer/* packages are forbidden — use agent-browser via Vision',
  },
  {
  ...
  { pattern: /\bplaywright\b/i, reason: 'Playwright is forbidden — use agent-browser via Vision' },
  { pattern: /\bpuppeteer\b/i, reason: 'Puppeteer is forbidden — use agent-browser via Vision' },
];
...
function findDenyMatch(args: unknown): { pattern: RegExp; reason: string } | null {
  ...
  const browserHit = BROWSER_AUTOMATION_DENY.find((d) => d.pattern.test(haystack));
  ...
  return {
    ...
    'tool.execute.before': async (input, output) => {
      ...
      if (hit) {
        ...
        const isBrowserAutomation = BROWSER_AUTOMATION_DENY.some((d) => d.pattern === hit.pattern);
        const prefix = isBrowserAutomation ? '[bob] BROWSER AUTOMATION GATE' : '[bob] LEGAL GATE';
    ...
    'tool.execute.after': async (input, output) => {
      ...
      if ((toolName === 'webfetch' || toolName === 'web_search') && output.output) {
        const haystack = JSON.stringify((input as { args?: unknown }).args ?? {});
