...
1. **Edit** the plan file \`.bob/plans/${planName}.md\`:
   - Change \`- [ ]\` to \`- [x]\` for the completed task
   - Use \`Edit\` tool to modify the checkbox

2. **Read** the plan file AGAIN:
   \`\`\`
   Read(".bob/plans/${planName}.md")
   \`\`\`
...
1. Read \
\`.bob/plans/${planName}.md\` again and confirm every remaining unchecked **top-level** task belongs to F1-F4.
