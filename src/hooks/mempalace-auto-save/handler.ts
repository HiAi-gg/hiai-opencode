...
export function createMemPalaceAutoSaveHandler(args: {
  ...
  const saveToMemPalace = async (params: {
    ...
    try {
      ...
      await skillMcpManager.callTool(info, context, "mempalace_add_drawer", {
        wing: params.wing,
        room: params.room,
  ...
  return async ({
    ...
    if (event.type === "session.idle" && sessionID) {
      try {
        const todosResp = await ctx.client.session.todo({
          path: { id: sessionID },
        });
        const todos =
          (
        ...
        const completedTodos = todos.filter(
          (t) =>
            t.status === "completed" && t.id && !state.savedTodos.has(t.id),
        );
          ...
          for (const todo of completedTodos) {
            if (todo.id) {
              ...
              if (content.toLowerCase().includes("plan")) {
                ...
                content.toLowerCase().includes("critic") ||
                ...
                content.toLowerCase().includes("stitch") ||
                content.toLowerCase().includes("handoff")
              ) {
                room = "designs";
                agent_name = "designer";
              }
    ...
    if (event.type === "session.error" && sessionID) {
      ...
      const errorDesc = error
        ? `${error.name ?? "Unknown error"}: ${error.message ?? "No message"}`
        : "Session error";
      if (canSave(sessionID)) {
        await saveToMemPalace({
          wing: "hiai-opencode",
