...
function hasRunningBackgroundTasks(sessionID: string, options?: GuardHookOptions): boolean {
  ...
  try {
    ...
    if (!canContinueSession) {
      log(`[${HOOK_NAME}] Skipped: tracked descendant agent does not match boulder agent`, {
        sessionID: input.sessionID,
        requiredAgent: currentBoulder.agent ?? "manager",
      })
      return
    }
...
export async function handleGuardSessionIdle(input: {
  ctx: PluginInput
  options?: GuardHookOptions
  ...
  if (!canContinueSession) {
    log(`[${HOOK_NAME}] Skipped: tracked descendant agent does not match boulder agent`, {
      sessionID,
      requiredAgent: boulderState.agent ?? "manager",
    })
    return
  }
...
async function canContinueTrackedBoulderSession(input: {
  ...
  const requiredAgentKey = getAgentConfigKey(input.requiredAgent ?? "manager")
  const sessionAgentKey = getAgentConfigKey(sessionAgent)
  return sessionAgentKey === requiredAgentKey
    || (requiredAgentKey === getAgentConfigKey("manager") && sessionAgentKey === getAgentConfigKey("bob"))
}
