...
import { isBobPath } from "./bob-path";
...
export function createToolExecuteBeforeHandler(input: {
  ...
  return async (toolInput, toolOutput): Promise<void> => {
    ...
    // Warn-only policy: Guard guides orchestrators toward delegation but doesn't block, allowing flexibility for urgent fixes
    if (isWriteOrEditToolName(toolInput.tool)) {
      ...
      if (filePath && !isBobPath(filePath)) {
    ...
    if (toolInput.tool === "task") {
      if (toolInput.callID) {
        ...
        if (requestedSessionId) {
          ...
          const planName = toolInput.sessionID
            ? findPlanNameForSession(ctx.directory, toolInput.sessionID)
            : null;
          const boulderState = planName
            ? readBoulderForPlan(ctx.directory, planName)
            : null;
