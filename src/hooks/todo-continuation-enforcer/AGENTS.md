...
14 files (~2061 LOC). The "boulder" — Continuation Tier hook that forces Bob to keep rolling when incomplete todos remain. Fires on `session.idle`, injects continuation prompt after 2s countdown toast.
...
```typescript
DEFAULT_SKIP_AGENTS = ["strategist", "compaction", "plan"]
...
`todoContinuationEnforcer` handles **main Bob sessions** only.
`guardHook` handles **boulder/ralph/subagent sessions** with a different decision gate.
