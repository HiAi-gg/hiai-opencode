...
export async function handleSessionIdle(args: {
  ...
  if (isRalphLoopActive) {
    ...
    try {
      ...
      loopActiveForSession =
        !!loopState?.active &&
        (loopState.session_id === undefined ||
          loopState.session_id === sessionID);
    } catch (error) {
      log(`[${HOOK_NAME}] ralph-loop state probe failed, continuing`, {
        sessionID,
        error: String(error),
      });
    }
  ...
  try {
    ...
    const freshTodos = normalizeSDKResponse(todoResp, [] as Todo[], {
      preferResponseOnMissingData: true,
    });
    ...
    if (decision.kind === "start" && startRalphLoop) {
      const started = startRalphLoop(sessionID, decision.goal, {
        ultrawork: true,
      });
      if (started) {
        state.autoLoopStarted = true;
        log(`[${HOOK_NAME}] Auto-started ralph-loop`, {
          sessionID,
          openTodos: decision.openTodos,
          threshold: autoLoopThreshold,
        });
        ctx.client.tui
          ?.showToast?.({
    ...
    log(`[${HOOK_NAME}] Auto-loop probe failed`, {
      sessionID,
      error: String(error),
    });
  }
