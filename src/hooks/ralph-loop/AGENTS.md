...
/ralph-loop → startLoop(sessionID, prompt, options)
  → loopState.startLoop() → persists state to .bob/ralph-loop.local.md
...
| `storage.ts` | Read/write `.bob/ralph-loop.local.md` state file |
...
```
.bob/ralph-loop.local.md  (gitignored)
