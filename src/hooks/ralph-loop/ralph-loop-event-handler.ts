...
import {
  ...
  readBoulderForPlan,
  syncBoulderNotepadsFromWorktree,
...
export function createRalphLoopEventHandler(
  ...
  return async ({
      ...
      try {
        ...
        if (state.verification_pending) {
          if (!verificationSessionID && matchesParentSession) {
            log(
              `[${HOOK_NAME}] Verification pending without tracked critic session, running recovery check`,
              {
                sessionID,
                iteration: state.iteration,
              },
        ...
        if (planName) {
          try {
            ...
            if (progress && progress.total === 0) {
              log(
                `[${HOOK_NAME}] Empty plan detected (0 tasks), stopping loop`,
                {
                  sessionID,
                  planName,
          ...
          if (planName) {
            ...
            if (existsSync(planPath)) {
              ...
              if (!hasCheckboxes && hasNumberedTasks) {
                log(
                  `[${HOOK_NAME}] Plan file lacks checkboxes but has numbered tasks`,
                  {
                    planName,
                    sessionID,
                  },
        ...
        if (elapsed < minIdleMs) {
          log(
            `[${HOOK_NAME}] Skipped: minimum idle delay not reached (${elapsed}ms < ${minIdleMs}ms)`,
            { sessionID },
          );
          return;
        }
    ...
    if (event.type === "session.deleted") {
      ...
      const deletedSessionID = (props as Record<string, unknown> | undefined)
        ?.sessionID as string | undefined;
      if (deletedSessionID) {
        const planName = findPlanNameForSession(
          options.directory,
          deletedSessionID,
        );
        if (planName) {
          try {
            ...
            if (planState?.worktree_path) {
              syncBoulderNotepadsFromWorktree(
                options.directory,
                planState.worktree_path,
              );
              deleteBoulderForPlan(options.directory, planName);
              log(
                `[${HOOK_NAME}] Synced notepads from deleted session worktree`,
                {
            ...
            log(
              `[${HOOK_NAME}] Failed to cleanup worktree on session deleted`,
              {
                error: String(err),
              },
            );
