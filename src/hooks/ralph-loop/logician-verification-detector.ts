...
const TASK_METADATA_PATTERN =
  /<task_metadata>[ \t]*([\s\S]*?)[ \t]*<\/task_metadata>/is;
...
function isVerificationGateAgent(agentName: string): boolean {
  return VERIFICATION_GATE_AGENT_KEYS.has(
    normalizeVerificationGateAgentKey(agentName),
  );
}

export function parseCriticVerificationEvidence(
  text: string,
): CriticVerificationEvidence | undefined {
...
export const parseLogicianVerificationEvidence =
  parseCriticVerificationEvidence;
...
export function isCriticVerified(text: string): boolean {
  ...
  if (evidence) {
    const isVerifiedPromise =
      evidence.promise === ULTRAWORK_VERIFICATION_PROMISE;
