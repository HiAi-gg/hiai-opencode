...
import {
  extractCriticSessionID,
  isCriticVerified,
} from "./logician-verification-detector";
...
async function detectCriticVerificationFromParentSession(
  ...
  try {
    ...
    log(
      `[${HOOK_NAME}] Failed to scan parent session for critic verification evidence`,
      {
        parentSessionID,
        error: String(error),
      },
  ...
  if (
    ...
    if (!verificationSessionID && state.session_id) {
      const recoveredVerificationSessionID =
        await detectCriticVerificationFromParentSession(
          ctx,
          state.session_id,
          directory,
          apiTimeoutMs,
