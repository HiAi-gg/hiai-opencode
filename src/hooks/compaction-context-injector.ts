...
export function createCompactionContextInjector(_config: BobConfig): HookSet {
  return {
    'experimental.session.compacting': async (
      ...
      output.context.push('[hiai-opencode] PRESERVE: Agent names and their current assignments.');
      output.context.push(
        '[hiai-opencode] PRESERVE: Any recovery context or error information if the session previously errored.',
      );
      output.context.push('[hiai-opencode] PRESERVE: File paths that were recently edited or created.');

      // Loop/recovery state markers
      output.context.push(
        '[hiai-opencode] PRESERVE: Loop iteration state if the session was in a multi-step workflow.',
      );
      output.context.push('[hiai-opencode] PRESERVE: Continuation prompts and pending instructions.');
    },
  };
}
