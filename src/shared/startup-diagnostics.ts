...
export function warnIfListPluginEntry(directory: string): void {
  ...
  for (const configPath of candidates) {
    ...
    logWarn(
      "This can prevent hiai-opencode MCP servers from loading from that config scope.",
    );
    logWarn(`Update it to: plugin: ["${PLUGIN_NAME}"]`);
  }
}

function hasConfigAuthFallback(
...
export function warnMissingRequiredMcpEnv(args: {
  ...
  for (const [name, entry] of Object.entries(HIAI_MCP_REGISTRY)) {
    ...
    const missing = entry.requiredEnv.filter(
      (envName) =>
        !process.env[envName]?.trim() &&
        !hasConfigAuthFallback(args.pluginConfig, envName),
    );

    if (missing.length === 0) continue;

    logWarn(
      `MCP "${name}" is enabled but missing required env: ${missing.join(", ")}.` +
