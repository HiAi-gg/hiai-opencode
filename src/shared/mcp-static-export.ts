...
export function buildStaticMcpJson(
  config: HiaiOpencodeConfig,
): StaticMcpJsonPayload {
...
export function autoExportStaticMcpJson(
  directory: string,
  config: HiaiOpencodeConfig,
): void {
  const mode =
    process.env.HIAI_OPENCODE_AUTO_EXPORT_MCP?.trim().toLowerCase() || "never";
  if (
    mode === "0" ||
    mode === "false" ||
    mode === "no" ||
  ...
  const outputPath =
    process.env.HIAI_OPENCODE_MCP_EXPORT_PATH?.trim() ||
    join(directory, ".opencode", ".mcp.json");
  ...
  if (
    ...
    !isManagedStaticMcpFile(outputPath) &&
    !isForceMode
  ) {
    logWarn(
      `refusing to overwrite non-managed static MCP config at ${outputPath}. ` +
