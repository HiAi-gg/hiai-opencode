...
export function captureLearn(
  entry: Omit<LearnEntry, "id" | "timestamp">,
): LearnEntry {
...
export function formatLearnEntriesForPrompt(learns: LearnEntry[]): string {
  ...
  const blocks = learns.map((l) => {
    const _agents = l.agents.join(", ") || "unknown";
    const tags = l.tags.join(", ") || "general";
    const conventions = l.conventions.length
      ? `  Conventions: ${l.conventions.join("; ")}`
      : "";
    const gotchas = l.gotchas.length
      ? `  Gotchas: ${l.gotchas.join("; ")}`
      : "";
    const failures = l.failures.length
      ? `  Avoid: ${l.failures.join("; ")}`
      : "";
