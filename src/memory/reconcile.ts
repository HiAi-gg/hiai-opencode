...
import { type MemoryLocator, parseCcFrontmatterType, parseCcPath, parsePath } from './paths';

const log = {
  warn: (msg: string, extra?: unknown) => console.warn(`[memory.reconcile] ${msg}`, extra ?? ''),
};
...
export async function indexFromDisk(
  ...
  if (!_dbPath) throw new Error('Memory DB path not set — call setMemoryDbPath() first');
...
export async function reconcileMemory(roots: { mimo: string; cc?: string }): Promise<{
  indexed: number;
  pruned: number;
}> {
  if (!_dbPath) throw new Error('Memory DB path not set — call setMemoryDbPath() first');
