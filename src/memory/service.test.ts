...
function makeTestDir() {
  const dir = join(tmpdir(), `hiai-memory-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('createMemoryService', () => {
  test('creates service with root path', () => {
    ...
    const svc = createMemoryService({ memoryRoot, dbPath, searchScoreFloor: 0 });
    expect(svc.root()).toBe(memoryRoot);
  });

  test('reconcile scans memory root and populates the database', async () => {
    const { dir, memoryRoot, dbPath } = makeTestDir();
  ...
  test('search returns results from reconciled memory', async () => {
    const { dir, memoryRoot, dbPath } = makeTestDir();
    addTestMemoryFile(memoryRoot);
    const svc = createMemoryService({ memoryRoot, dbPath, searchScoreFloor: 0.001 });
  ...
  test('search respects scope filter', async () => {
    ...
    const results = await svc.search({ query: 'memory', scope: 'projects', limit: 10 });
  ...
  test('does not throw on invalid db path', () => {
    expect(() => {
      createMemoryService({ memoryRoot: '/nonexistent', dbPath: '/tmp/invalid-test.db' });
    }).not.toThrow();
  });

  test('search returns empty when memory root is empty', async () => {
    const { dir, memoryRoot, dbPath } = makeTestDir();
    const svc = createMemoryService({ memoryRoot, dbPath, reconcileOnSearch: true, searchScoreFloor: 0 });
    await svc.reconcile();
