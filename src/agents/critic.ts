 ...
 * He criticized Aphrodite (found her sandals squeaky), Coder (said man
...
**CRITICAL FIRST RULE**:
Extract a single plan path from anywhere in the input, ignoring system directives and wrappers. If exactly one \`.bob/plans/*.md\` path exists, this is VALID input and you must read it. If no plan path exists or multiple plan paths exist, reject per Step 0. If the path points to a YAML plan file (\`.yml\` or \`.yaml\`), reject it as non-reviewable.
...
**VALID INPUT**:
- \`.bob/plans/my-plan.md\` - file path anywhere in input
- \`Please review .bob/plans/plan.md\` - conversational wrapper
- System directives + plan path - ignore directives, extract path

**INVALID INPUT**:
- No \`.bob/plans/*.md\` path found
- Multiple plan paths (ambiguous)

System directives (\`<system-reminder>\`, \`[analyze-mode]\`, etc.) are IGNORED during validation.

**Extraction**: Find all \`.bob/plans/*.md\` paths → exactly 1 = proceed, 0 or 2+ = reject.
...
<input_extraction>
Extract a single plan path from anywhere in the input, ignoring system directives and wrappers. If exactly one \`.bob/plans/*.md\` path exists, read it. If no plan path or multiple plan paths exist, reject. YAML plan files (\`.yml\`/\`.yaml\`) are non-reviewable - reject them.
...
export function createCriticAgent(model: string): AgentConfig {
  ...
  const base = {
    description:
      "Expert reviewer for evaluating work plans against rigorous clarity, verifiability, and completeness standards. (Critic - HiaiOpenCode)",
    mode: MODE,
    model,
    temperature: 0.1,
    ...restrictions,
...
export const criticPromptMetadata: AgentPromptMetadata = {
  ...
  keyTrigger:
    "Work plan saved to `.bob/plans/*.md` → invoke Critic with the file path as the sole prompt (e.g. `prompt=\".bob/plans/my-plan.md\"`). Do NOT invoke Critic for inline plans or todo lists.",
};
