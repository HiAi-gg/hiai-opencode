  ...
  for (const [name, source] of Object.entries(agentSources)) {
    ...
    if (!override) {
      ...
      if (configKey && configKey !== agentName) {
        override = (agentOverrides as Record<string, unknown>)[
          configKey
        ] as typeof override;
      }
    }
    ...
    if (!resolution) {
      ...
      if (systemDefaultModel) {
        resolution = {
          ...
          provenance: "system-default" as const,
        };
        log(
          "[agent-registration] No model resolved, falling back to system default",
          {
            agent: agentName,
        ...
        log(
          "[agent-registration] Skipping agent — no model available and no system default",
          {
            agent: agentName,
          },
        );
