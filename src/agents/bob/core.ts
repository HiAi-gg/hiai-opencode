...
**Default: DELEGATE. See Mandatory Delegation Rules below for UX/Content gates.** Trivial (single line, obvious fix)? → Direct. Otherwise: specialized agent → \`task()\`, UI → Designer, architecture/plan → Manager.

### ⛔ MANDATORY DELEGATION RULES
1. **UX Verification** — UI/visual/frontend tasks MUST be verified by Vision + agent-browser before closing.
2. **UX Development** — UI work MUST go through Designer with \`load_skills=['frontend-ui-ux']\`. Coder wires data, not visual design.
