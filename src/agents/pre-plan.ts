...
/**
 * Pre-Plan - Plan Consultant Agent
 *
 * Named after the Greek goddess of wisdom, prudence, and deep counsel.
 * Pre-Plan analyzes user requests BEFORE planning to prevent AI failures.
 *
...
export const METIS_SYSTEM_PROMPT = `# Pre-Plan - Pre-Planning Consultant
...
- **Architecture**: "how should we structure", system design, infrastructure - STRATEGIC: long-term impact, Logician recommendation
...
**Logician Consultation** (RECOMMEND to Strategist):
\`\`\`
Task(
...
**Directives for Strategist**:
- MUST: Consult Logician before finalizing plan
...
- **\`logician\` agent**: Read-only consultation. High-IQ debugging, architecture - Architecture
...
const prePlanRestrictions = createAgentToolRestrictions([
  "write",
  "edit",
  "apply_patch",
  "task",
...
export function createPrePlanAgent(model: string): AgentConfig {
  return {
    description:
      "Pre-planning consultant that analyzes requests to identify hidden intentions, ambiguities, and AI failure points. (Pre-Plan - HiaiOpenCode)",
    mode: MODE,
    model,
    temperature: 0.3,
    ...prePlanRestrictions,
...
createPrePlanAgent.mode = MODE

export const prePlanPromptMetadata: AgentPromptMetadata = {
  category: "advisor",
  cost: "EXPENSIVE",
  triggers: [
    {
  ...
  promptAlias: "Pre-Plan",
  keyTrigger: "Ambiguous or complex request → consult Pre-Plan before Strategist",
}
