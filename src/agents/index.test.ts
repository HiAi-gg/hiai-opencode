...
import { applyPromptOverride, createAllAgents, resolveAgentModel } from './index';
...
describe('agent bob.json overrides', () => {
  test('appends prompt_append to the final agent prompt', () => {
    ...
    expect(bob?.config.prompt).toContain('Always report the verification command.');
  });
  ...
  test('does not add an empty prompt suffix', () => {
    expect(applyPromptOverride('bob', 'base prompt', { agent_overrides: { bob: { prompt_append: '  ' } } })).toBe(
      'base prompt',
    );
  });
});
