...
describe('VISION_PROMPT', () => {
  ...
  test('contains MUST call at least one agent_browser_* tool instruction', () => {
    expect(VISION_PROMPT).toContain('MUST call at least one agent_browser_* tool');
  });
  ...
  test('contains Do NOT report results derived from code-reading', () => {
    expect(VISION_PROMPT).toContain('Do NOT report results derived from code-reading');
    expect(VISION_PROMPT).toContain('static analysis');
  });

  test('contains verdict without browser tool evidence is INVALID', () => {
    expect(VISION_PROMPT).toContain('verdict without browser tool evidence is INVALID');
  ...
  test('contains ABSOLUTE PROHIBITION for Playwright/Puppeteer in Constraints', () => {
    ...
    expect(VISION_PROMPT).toMatch(/Playwright.*Puppeteer.*FORBIDDEN|FORBIDDEN.*Playwright.*Puppeteer/i);
  });

  test('forbids Playwright and Puppeteer explicitly', () => {
    expect(VISION_PROMPT).toMatch(/\bplaywright\b.*FORBIDDEN|FORBIDDEN.*\bplaywright\b/i);
    expect(VISION_PROMPT).toMatch(/\bPuppeteer\b.*FORBIDDEN|FORBIDDEN.*\bPuppeteer\b/i);
  });
...
describe('formatScreenshotOutput', () => {
  ...
  test('persists base64 PNG to disk and returns path with size', () => {
    ...
    expect(result).toContain('.bob/screenshots/agent-browser-');
  ...
  test('truncates long text output over 5000 chars', () => {
    ...
    const longText =
      'Console log line ' +
      Array.from({ length: 600 }, (_, i) => `[${i}] element found: <div class="container">\n`).join(
        '',
      );
  ...
  test('passes through JSON output unchanged (if short enough)', () => {
    const jsonText = JSON.stringify({ path: '/tmp/screenshot.png', size: 12345 });
