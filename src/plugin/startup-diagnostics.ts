...
export function lintModeAgentCapabilities(): void {
  for (const [mode, agent] of Object.entries(MODE_TO_AGENT)) {
    if (WRITE_MODES.has(mode) && !WRITE_CAPABLE_AGENTS.has(agent)) {
      console.warn(
        `[startup-lint] Mode "${mode}" targets agent "${agent}" which cannot write/edit. Tasks using this mode may fail when attempting file operations.`,
      );
    }

    if (READONLY_MODES.has(mode) && WRITE_CAPABLE_AGENTS.has(agent)) {
      console.warn(
        `[startup-lint] Mode "${mode}" targets agent "${agent}" which can write. Consider verifying this is intentional.`,
      );
    }
  }
}
