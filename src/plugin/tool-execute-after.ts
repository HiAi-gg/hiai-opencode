...
function normalizeAgentKey(agentName: string | undefined): string | undefined {
  ...
  const normalized = stripInvisibleAgentCharacters(getAgentConfigKey(stripped))
    .trim()
    .toLowerCase();
  ...
  return async (
    ...
    if (input.tool === "task") {
      ...
      if (
        ...
        log(
          "[tool-execute-after] Stored critic verification session via attempt match",
          {
            parentSessionID: input.sessionID,
            criticSessionID: sessionId,
            verificationAttemptId,
        ...
        log(
          "[tool-execute-after] Fallback: stored critic verification session without attempt match",
          {
            parentSessionID: input.sessionID,
            criticSessionID: sessionId,
            hasPromptInMetadata: typeof prompt === "string",
    ...
    const runToolExecuteAfterHooks = async (): Promise<void> => {
      ...
      await hooks.guardHook?.["tool.execute.after"]?.(input, output);
