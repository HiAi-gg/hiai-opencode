...
export async function handleSessionStatus(
  ...
  if (sessionID && status?.type === "retry" && deps.isModelFallbackEnabled && !deps.isRuntimeFallbackEnabled) {
    try {
      ...
      if (shouldRetryError(errorInfo)) {
        ...
        if (agentName) {
          ...
          const currentProvider = deps.resolveFallbackProviderID(sessionID, parsed.providerID);
