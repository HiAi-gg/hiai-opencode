...
- Doctor plugin-registration tests are Windows-safe: project-local `.opencode/opencode.json`, `c7.cmd`/`opencode.cmd` stubs, and `path.delimiter` on PATH.
...
- Added `plan-invocation-injector` hook and updated plan skills.

## [0.4.10] — 2026-07-18

### 🪟 Windows CI signal semantics

- Skip the POSIX SIGTERM exit-code regression on Windows, where Bun's `child.kill("SIGTERM")` is a no-op and cannot exercise the runner's POSIX signal handler. Linux and macOS retain the strict exit-code `2` assertion.

## [0.4.9] — 2026-07-18

### 🪟 Windows CI compatibility

- Made the npm-package-runner signal regression assertion platform-aware: POSIX keeps the strict exit code `2`, while Windows validates non-success termination because Node cannot deliver POSIX SIGTERM handlers there.

## [0.4.8] — 2026-07-18

### 🧪 MCP runner test stability

- Hardened the npm-package-runner signal regression test against slow process startup by scheduling termination only after the runner has spawned.
